...
func TestDomainAvoidRulesDatabasePathsWithoutDatabase(
	t *testing.T,
) {
	ctx := context.Background()

	firstCreated := time.Date(
		2026,
		time.September,
		18,
		18,
	...
	t.Run(
		...
		func(t *testing.T) {
			testErr := errors.New(
				"test domain rules query failure",
			)
			...
			if !errors.Is(err, testErr) ||
				!strings.Contains(
					err.Error(),
					"store: list crawl domain avoid rules",
				) {
				t.Fatalf(
					"DomainAvoidRules() error = %v",
					err,
